...
#define MAX_BUFFER 12     //Max size of buffer in one cycle
#define ITEMS_PROD 12     //Max items to be produced
#define ITEMS_CONS 6      //Max items to be consumed
...
void *Produce(void * n)                   //Producer thread to produce item
{
  ...
  for (int i = 0; i < ITEMS_PROD; i++) 
  {
    ...
    in = (in + 1)%(MAX_BUFFER);                             //Mod taken to ensure cyclicity
    sem_signal(&mx);
    sem_signal(&full);
  }
}
...
void *Consume(void * n)                         //Consumer thread to consume item
{
  for (int i = 0; i < ITEMS_CONS; i++)
  {
    ...
    out = (out + 1)%(MAX_BUFFER);                 //Mod taken to ensure cyclicity
    sem_signal(&mx);
    sem_signal(&empty);
  }
}
